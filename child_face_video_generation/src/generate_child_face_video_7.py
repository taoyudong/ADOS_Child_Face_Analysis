...
def generate_child_faces(video_path, fd_rst, threshold=15.0, batch_size=32, cache_size=320):
    ...
    try:
        for img_batch, meta_batch, video_frame_batch in loader:
            ...
            for face, meta, predicted_age in zip(video_frame_batch, meta_batch, predicted_ages):
                if predicted_age <= threshold:
                    child_faces.append(face)
                    face_meta[len(face_meta)] = meta[1]
                    face_meta[len(face_meta)-1]['frame_id'] = meta[0]
                    face_meta[len(face_meta)-1]['estimated_age'] = predicted_age
    except KeyError as e:
        print(e)
        
    return child_faces, face_meta
...
if __name__ == '__main__':
    ...
    video_dir = Path('/home/charley/Research/Yudong_Psycho/Videos/')

    # Path to the outputs
    # output_dir = Path('/Users/yudongtao/Research/ADOS_Analysis/data/child_face_videos')
    output_dir = Path('/home/mitch-b/dmis-research/Yudong/2019/Psychology/Expression/ADOS_Analysis/data/child7')
    ...
    for file in sorted(video_dir.iterdir()):
        if file.suffix in ['.mp4', '.MP4']:
            ...
            if not fd_result_path.exists():    
                ...
                child_faces, meta = generate_child_faces(file, fd_result, threshold=7.)
